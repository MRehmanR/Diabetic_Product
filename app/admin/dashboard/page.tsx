...
export default function AdminDashboardPage() {
  ...
  async function handleFormSubmit(formData: SupplyFormData & { slug: string }) {
    ...
    if (editingSupply) {
      const { error } = await supabase.from("supplies").update(payload).eq("id", editingSupply.id);
      if (error) {
  ...
  async function handleDelete() {
    ...
    const { error } = await supabase.from("supplies").delete().eq("id", deleteSupply.id);
    if (error) {
  ...
  async function updateOfferStatus(offer: Offer, status: string, quotedAmount?: number) {
    ...
    const { error } = await supabase.from("offers").update(update).eq("id", offer.id);
    if (error) {
